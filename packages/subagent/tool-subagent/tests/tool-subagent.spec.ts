...
describe('dsh-tool-subagent', () => {
  ...
  it('passes persona/toolFilter/maxDepth config through to the start request', async () => {
    ...
    ctx.subagents.registerProvider({
      name: 'capture2',
      capabilities: { outputSchema: false, depthLimit: true, toolFilter: true, persona: true, agentPreset: true },
      inheritsParentContext: false,
      start: async (request) => {
        seen = request
        return {
  ...
  it('exposes profile selection only when enabled and forwards the selected preset', async () => {
    ...
    ctx.subagents.registerProvider({
      name: 'profile-aware',
      capabilities: { outputSchema: false, depthLimit: false, toolFilter: false, persona: false, agentPreset: true },
      inheritsParentContext: false,
      start: async (request) => {
        seen = request
        return {
          id: SessionId('profile-aware-child'),
          localAgent: undefined,
          result: Promise.resolve({ output: [{ type: 'text', text: 'ok' }], stopReason: 'completed' as const }),
          dispose: async () => {},
        }
    ...
    await ctx.plugin(tool, { provider: 'profile-aware', allowAgentPreset: true, maxDepth: 'provider-managed' })

    const props = (ctx.tools.schemas().find(s => s.name === 'subagent')!.parameters as { properties: Record<string, unknown> }).properties
    expect(props.profile).toBeDefined()
    await callSubagent(ctx, { description: 'd', prompt: 'p', profile: 'minimal' })
    expect(seen?.agentPreset).toBe('minimal')
  })
  ...
  it('a partial toolFilter (deny only) does not materialize an empty allow-list (deny-all trap)', async () => {
    ...
    ctx.subagents.registerProvider({
      name: 'capture3',
      capabilities: { outputSchema: false, depthLimit: false, toolFilter: true, persona: false, agentPreset: false },
      inheritsParentContext: false,
      start: async (request) => {
        seen = request
        return {
