...
function install(agent, ctx, config) {
	...
	try {
		const denied = ctx.agentTeams.membership(agent).role === "lead" ? [...config.denyTools, ...config.denyLeadTools] : config.denyTools;
...
function apply(ctx, config = {}) {
	...
	const reconcile = (agent) => {
		const matchesPreset = config.agentPreset === void 0 || resolveSessionPreset(agent.session) === config.agentPreset;
		const membership = ctx.agentTeams.tryMembership(agent);
		if (!matchesPreset || membership === void 0) {
			installed.get(agent)?.();
	...
	ctx.on("session/event", (session, event) => {
		...
		if (agent !== void 0) reconcile(agent);
	});
